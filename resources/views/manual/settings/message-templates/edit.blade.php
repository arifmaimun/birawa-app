    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <form method="POST" action="{{ route('manual.message-templates.update', $messageTemplate) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <x-manual.text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title', $messageTemplate->title)" required autofocus />
                        ...
                        <div>
                            ...
                            <textarea id="content_pattern" name="content_pattern" rows="5" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>{{ old('content_pattern', $messageTemplate->content_pattern) }}</textarea>
                        ...
                        <div>
                            ...
                            <select id="trigger_event" name="trigger_event" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                <option value="on_departure" {{ old('trigger_event', $messageTemplate->trigger_event) == 'on_departure' ? 'selected' : '' }}>On Departure</option>
                                <option value="on_arrival" {{ old('trigger_event', $messageTemplate->trigger_event) == 'on_arrival' ? 'selected' : '' }}>On Arrival</option>
                            </select>
