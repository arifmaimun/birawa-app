...
<div class="max-w-2xl mx-auto bg-white shadow rounded-lg">
    ...
    <div class="p-6">
        <form action="{{ route('manual.visits.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="datetime-local" name="scheduled_at" id="scheduled_at" value="{{ old('scheduled_at') }}" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                @error('scheduled_at')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="visit_status_id" id="visit_status_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md" required>
                    @foreach($statuses as $status)
                        <option value="{{ $status->id }}" {{ old('visit_status_id') == $status->id ? 'selected' : '' }}>
                            {{ $status->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <textarea name="complaint" id="complaint" rows="3" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('complaint') }}</textarea>
                @error('complaint')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="mt-1 relative rounded-md shadow-sm">
                    ...
                    <input type="number" name="transport_fee" id="transport_fee" value="{{ old('transport_fee', 0) }}" class="focus:ring-blue-500 focus:border-blue-500 block w-full pl-12 sm:text-sm border-gray-300 rounded-md" placeholder="0">
                </div>
                @error('transport_fee')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
