    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <form method="POST" action="{{ route('manual.patients.update', $patient) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <x-manual.text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $patient->name)" required />
                        ...
                        <div>
                            ...
                            <select id="species" name="species" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                <option value="Cat" {{ old('species', $patient->species) == 'Cat' ? 'selected' : '' }}>Cat</option>
                                <option value="Dog" {{ old('species', $patient->species) == 'Dog' ? 'selected' : '' }}>Dog</option>
                                <option value="Bird" {{ old('species', $patient->species) == 'Bird' ? 'selected' : '' }}>Bird</option>
                                <option value="Other" {{ old('species', $patient->species) == 'Other' ? 'selected' : '' }}>Other</option>
                            </select>
                        ...
                        <div>
                            ...
                            <x-manual.text-input id="breed" class="block mt-1 w-full" type="text" name="breed" :value="old('breed', $patient->breed)" />
                        ...
                        <div>
                            ...
                            <x-manual.text-input id="dob" class="block mt-1 w-full" type="date" name="dob" :value="old('dob', $patient->dob ? $patient->dob->format('Y-m-d') : '')" />
                            <x-manual.input-error :messages="$errors->get('dob')" class="mt-2" />
                        </div>

                        <!-- Sterile -->
                        <div class="flex items-center mt-6">
                            <input id="is_sterile" type="checkbox" name="is_sterile" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" {{ old('is_sterile', $patient->is_sterile) ? 'checked' : '' }}>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="allergies" name="allergies" rows="2" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('allergies', $patient->allergies) }}</textarea>
