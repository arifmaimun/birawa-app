<x-app-layout>
    ...
    <div class="max-w-3xl px-4 py-6 mx-auto" x-data="clientForm({{ \Illuminate\Support\Js::from(old('addresses', $defaultAddresses)) }})">
        <form action="{{ route('clients.store') }}" method="POST" @submit="isSubmitting = true">
            @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2 pt-4 border-t border-gray-100">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div x-data="dropdownInput({{ \Illuminate\Support\Js::from($idTypes) }}, '{{ old('id_type') }}')">
                                ...
                                <!-- ID Number -->
                                <div>
                                    <label for="id_number" class="block mb-1 text-xs font-bold text-gray-500 uppercase">Nomor Identitas</label>
                                    <input type="text" name="id_number" id="id_number" class="block w-full px-3 py-2.5 text-gray-900 bg-white border border-gray-300 rounded-lg focus:ring-birawa-500 focus:border-birawa-500 sm:text-sm" placeholder="Nomor KTP / Passport / SIM">
                                </div>
                                ...
                                <div x-data="dropdownInput({{ \Illuminate\Support\Js::from($ethnicities) }}, '{{ old('ethnicity') }}')">
                                ...
                                <!-- Religion -->
                                <div x-data="dropdownInput({{ \Illuminate\Support\Js::from($religions) }}, '{{ old('religion') }}')">
                                    <label class="block mb-1 text-xs font-bold text-gray-500 uppercase">Agama</label>
                                    <input type="hidden" name="religion" :value="finalValue">
                                    <div x-show="!isCustom">
                                        <select x-model="selectedValue" class="block w-full px-3 py-2.5 text-gray-900 bg-white border border-gray-300 rounded-lg focus:ring-birawa-500 focus:border-birawa-500 sm:text-sm">
                                    ...
                                    <div x-show="isCustom" class="flex gap-2">
                                        <input type="text" x-model="customValue" class="block w-full px-3 py-2.5 text-gray-900 bg-white border border-blue-500 rounded-lg focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Masukkan Agama Baru">
                                        <button type="button" @click="reset()" class="px-3 py-2 text-gray-500 bg-gray-100 rounded-lg hover:bg-gray-200">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                        </button>
                                    </div>
            ...
            <div class="mb-6 overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/50">
                    ...
                    <button type="button" @click="addAddress" class="text-sm font-bold text-birawa-600 bg-birawa-50 px-3 py-2 rounded-lg hover:bg-birawa-100 transition-colors flex items-center gap-1">
                ...
                <div class="p-6 space-y-8">
                    ...
                    <template x-for="(address, index) in addresses" :key="index">
                        <div class="relative p-4 border border-gray-200 rounded-xl bg-gray-50/30">
                            <!-- Remove Button -->
                            <button type="button" @click="removeAddress(index)" x-show="addresses.length > 1" class="absolute top-2 right-2 p-2 text-red-500 bg-red-50 rounded-lg hover:bg-red-100 hover:text-red-700 transition-colors" title="Hapus Alamat">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </button>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div x-data="dropdownInput({{ \Illuminate\Support\Js::from($parkingTypes) }}, address.parking_type)">
                                ...
                                <div x-data="dropdownInput({{ \Illuminate\Support\Js::from($addressTypes) }}, address.address_type)">
            ...
            <!-- Section 3: Informasi Hewan Peliharaan -->
            <x-patients.form-fields 
                :clients="collect([])" 
                prefix="patient_" 
                :hideClientSelection="true" 
                :withCard="true" 
            />
            ...
            <div class="mt-8 flex flex-col md:flex-row justify-end items-center gap-4">
                <button type="submit" 
                    ...
                    class="w-full md:w-auto md:min-w-[200px] px-6 py-3 font-bold text-white bg-birawa-600 rounded-xl hover:bg-birawa-700 transition-colors flex justify-center items-center gap-2 min-h-[44px]">
                    <span x-show="isSubmitting" class="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></span>
                    <span x-text="isSubmitting ? 'Menyimpan...' : 'Selesai'"></span>
                </button>
            </div>
        </form>
    </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('clientForm', (initialAddresses) => ({
                isSubmitting: false,
                isBusiness: false,
                dobMode: 'date',
                dobDate: '',
                ...
                init() {
                    // Ensure every address has an ID for Alpine :key
                    this.addresses = this.addresses.map(addr => {
                        if (!addr.id) addr.id = Date.now() + Math.random();
                        return addr;
                    });
                },

                addAddress() {
                    this.addresses.push({ id: Date.now() + Math.random(), street: '', additional_info: '', city: '', province: '', postal_code: '', country: 'Indonesia', parking_type: '', address_type: '' });
                },
