        ...
        <form action="{{ route('patients.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="owner_id" id="owner_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('owner_id') border-red-500 @enderror" required>
                    ...
                    @foreach($owners as $owner)
                        <option value="{{ $owner->id }}" {{ (old('owner_id') == $owner->id || (isset($selectedOwnerId) && $selectedOwnerId == $owner->id)) ? 'selected' : '' }}>{{ $owner->name }}</option>
                    @endforeach
                </select>
                @error('owner_id')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="mb-4">
                    ...
                    <input type="text" name="species" id="species" placeholder="Kucing, Anjing, dll" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('species') border-red-500 @enderror" value="{{ old('species') }}" required>
                    @error('species')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="breed" id="breed" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('breed') border-red-500 @enderror" value="{{ old('breed') }}">
                    @error('breed')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="dob" id="dob" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('dob') border-red-500 @enderror" value="{{ old('dob') }}">
                    @error('dob')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
