<x-app-layout>
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="mb-6">
            ...
            <h2 class="text-2xl font-bold text-slate-800">Edit Template Pesan</h2>
        </div>
            ...
            <form action="{{ route('message-templates.update', $messageTemplate) }}" method="POST">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title', $messageTemplate->title)" required autofocus />
                    ...
                    <div>
                        ...
                        <select id="trigger_event" name="trigger_event" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-birawa-500 focus:ring-birawa-500">
                            ...
                            <option value="on_departure" {{ old('trigger_event', $messageTemplate->trigger_event) == 'on_departure' ? 'selected' : '' }}>Saat Dokter Berangkat (Tombol "Mulai Berangkat")</option>
                            <option value="on_arrival" {{ old('trigger_event', $messageTemplate->trigger_event) == 'on_arrival' ? 'selected' : '' }}>Saat Dokter Sampai (Tombol "Sampai")</option>
                        </select>
                    ...
                    <div>
                        ...
                        <textarea id="content_pattern" name="content_pattern" rows="6" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-birawa-500 focus:ring-birawa-500" required>{{ old('content_pattern', $messageTemplate->content_pattern) }}</textarea>
