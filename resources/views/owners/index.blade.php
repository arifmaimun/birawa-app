            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div class="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex flex-col gap-3 relative overflow-hidden">
                        ...
                        <div class="flex justify-between items-start z-10">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 class="font-bold text-slate-800 text-lg leading-tight">{{ $owner->name }}</h3>
                                    <p class="text-xs text-slate-500">Joined {{ $owner->created_at->format('M Y') }}</p>
                                </div>
                            </div>
                        ...
                        <div class="space-y-2">
                            ...
                            @if($owner->address)
                                <div class="flex items-start gap-2 text-sm text-slate-600">
                                    ...
                                    <span class="line-clamp-2">{{ $owner->address }}</span>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-2 mt-2 pt-3 border-t border-slate-100">
                            <a href="{{ route('owners.edit', $owner) }}" class="flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-birawa-600 bg-birawa-50 rounded-lg hover:bg-birawa-100 transition-colors">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                                Edit
                            </a>
                            <form action="{{ route('owners.destroy', $owner) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this owner?');" class="w-full">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <div class="col-span-1 md:col-span-2 lg:col-span-3 text-center py-12">
                        ...
                        <p class="mt-1 text-slate-500">Try adjusting your search or add a new owner.</p>
                        <div class="mt-6">
                            <a href="{{ route('owners.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-xl text-white bg-birawa-600 hover:bg-birawa-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-birawa-500">
