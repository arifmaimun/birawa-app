<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-slate-800">Owner Details</h1>
            <a href="{{ route('owners.index') }}" class="text-sm font-medium text-birawa-600 hover:text-birawa-700 flex items-center gap-1">
        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 mb-8 relative overflow-hidden">
            ...
            <div class="flex flex-col md:flex-row md:items-start justify-between gap-6 relative z-10">
                <div class="flex gap-5">
                    ...
                    <div>
                        <h2 class="text-2xl font-bold text-slate-800 mb-1">{{ $owner->name }}</h2>
                        <div class="flex flex-col gap-1.5">
                            ...
                            @if($owner->email && !str_contains($owner->email, '@birawa.vet'))
                            <div class="flex items-center gap-2 text-slate-600 text-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="flex flex-row md:flex-col gap-3 mt-4 md:mt-0 w-full md:w-auto">
                    <a href="{{ route('owners.edit', $owner) }}" class="flex-1 md:flex-none px-5 py-2.5 bg-slate-100 text-slate-700 rounded-xl text-sm font-bold hover:bg-slate-200 text-center flex items-center justify-center gap-2 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                        </svg>
                        Edit Profile
                    </a>
                    <form action="{{ route('owners.destroy', $owner) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this owner? This will remove all their pets and records.');" class="flex-1 md:flex-none">
                        @csrf 
                        @method('DELETE')
        ...
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-xl font-bold text-slate-800 flex items-center gap-2">
                Pets
                <span class="px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-600 text-xs font-bold">{{ $owner->pets->count() }}</span>
            </h2>
            <a href="{{ route('patients.create') }}?owner_id={{ $owner->id }}" class="px-5 py-2.5 bg-birawa-500 text-white rounded-xl text-sm font-bold shadow-lg shadow-birawa-500/30 hover:bg-birawa-600 flex items-center gap-2 transition-all active:scale-95">
