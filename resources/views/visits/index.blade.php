<x-app-layout>
    <div x-data="{ view: 'list' }">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex space-x-2">
                <div class="bg-gray-200 rounded-lg p-1 flex">
                    <button @click="view = 'list'" :class="{ 'bg-white shadow': view === 'list', 'text-gray-600': view !== 'list' }" class="px-4 py-2 rounded-md text-sm font-medium transition-colors">
                        List View
                    </button>
                    <button @click="view = 'calendar'; setTimeout(() => { window.calendar.render() }, 100)" :class="{ 'bg-white shadow': view === 'calendar', 'text-gray-600': view !== 'calendar' }" class="px-4 py-2 rounded-md text-sm font-medium transition-colors">
                        Calendar View
                    </button>
                </div>
                <a href="{{ route('visits.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded ml-4">
                    + Jadwalkan Kunjungan
                </a>
            </div>
        </div>
        ...
        <!-- Calendar View -->
        <div x-show="view === 'calendar'" class="bg-white rounded-lg shadow p-6" style="display: none;">
            <div id="calendar"></div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Don't render immediately, wait for tab switch
        });
    </script>
    @endpush
</x-app-layout>
