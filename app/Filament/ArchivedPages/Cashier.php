...
class Cashier extends Page
{
    ...
    {
        if (empty($this->promoCodeInput)) return;

        $promo = \App\Models\Promo::where('code', $this->promoCodeInput)->first();

        if (!$promo) {
            Notification::make()->danger()->title('Invalid Promo Code')->send();
            return;
        }
        
        // Calculate subtotal first to check validity
        $tempSubtotal = 0;
        foreach ($this->cart as $item) {
            $tempSubtotal += $item['price'] * $item['qty'];
        }

        if (!$promo->isValid($tempSubtotal)) {
             Notification::make()->warning()->title('Promo conditions not met')->send();
             return;
        }

        $this->appliedPromoId = $promo->id;
        $this->calculateTotal();
        Notification::make()->success()->title('Promo Applied')->send();
    }
    ...
    public function calculateTotal()
    {
        ...
        if ($this->appliedPromoId) {
            $promo = \App\Models\Promo::find($this->appliedPromoId);
            if ($promo && $promo->isValid($this->subtotal)) {
                $this->discountAmount = $promo->calculateDiscount($this->subtotal);
            } else {
                $this->appliedPromoId = null; // Reset if no longer valid
            }
        }
    ...
    public function checkout(InventoryService $inventoryService)
    {
        ...
        DB::transaction(function () use ($inventoryService) {
            ...
            if ($this->appliedPromoId) {
                \App\Models\Promo::where('id', $this->appliedPromoId)->increment('used_count');
            }
